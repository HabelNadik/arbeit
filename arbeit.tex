...
PLCC ist die \textit{Software}, die die Daten der PTGL generiert und verwaltet. 


\paragraph{Die Berechnung der Graphen der PTGL}

erfolgt unter Verwendung der entsprechenden PDB und DSSP Dateien. Um den Graphen f\"ur eine Polypeptidkette zu berechnen werden aus der DSSP-Datei die Sekund\"arstrukturelemente (SSEs) des Proteins ausgelesen. F\"ur jedes Paar von SSEs wird die Anzahl der r\"aumlichen Kontakte ihrer Residuen berechnet. Wenn die Anzahl dieser Kontakte einen gewissen Grenzwert \"uberschreitet, wird angenommen, dass diese SSEs r\"aumlich benachbart sind und die jeweiligen Knoten werden durch eine Kante verbunden. So wird f\"ur jede Polypeptidkette ein Graph erstellt.
...
\paragraph{Aminos\"auregraphen} werden analog zu Proteingraphen und Komplexgraphen berechnet. Der Unterschied zu den anderen Graphformaten ist, das keine SSEs betrachtet werden. Stattdessen repr\"asentiert jeder Knoten eine Aminos\"aure eines Proteins. Die Knoten werden entsprechend der chemischen Eigenschaften der Aminos\"auren markiert. Knoten, die saure oder basische Residuen darstellen werden mit einem c markiert. Ein p markiert Knoten f\"ur polare Residuen, die weder sauer noch basisch sind. F\"ur unpolare Aminos\"auren wird ein h verwendet. Auch Liganden k\"onnen in Aminos\"auregraphen dargestellt werden. Ihre Knoten werden durch ein ? markiert. Aminos\"auregraphen k\"onnen Proteinkomplexe und einzelne Polypeptidketten darstellen.


Weiterhin erm\"oglicht PLCC den Vergleich von \textit{Graphlet}-Vektoren, die im folgenden Teil vorgestellt werden. Diese werden mit der von \textit{Pr{\v u}lic et al.} \cite{frqdistribution} vorgestellten Relativen-\textit{Graphlet}-H\"aufigkeiten-Distanz verglichen.


\section{Der \textit{Graphlet}-Algorithmus}


Um die Graphen der PTGL vergleichen zu k\"onnen werden \textit{Graphlets} verwendet. Diese sind im Gegensatz zu Graph-Isomorphismen in polynomieller Zeit berechenbar \cite{sherv_graphlets}. \textit{Graphlets} sind kleine, induzierte, zusammenh\"angende Teilgraphen mit bis zu 5 Knoten. Die Abbildungen \ref{fig:3graphlets}, \ref{fig:4graphlets} und \ref{fig:5graphlets} zeigen die \textit{Graphlets} mit 3, 4 und 5 Knoten. Sie werden durch den Algorithmus gez\"ahlt, so dass anstatt von Graphen nur noch Vektoren mit den \textit{Graphlet}-H\"aufigkeiten verglichen werden. Ein solcher Vergleich von Vektoren ist in konstanter Zeit m\"oglich.

\paragraph{Beschreibung des Algorithmus}
Um alle \textit{Graphlets} der Gr\"o"se $k \in \{3,4,5\}$ zu z\"ahlen werden alle Euler-Wege der L\"ange $k-1$ in dem gegebenen Graphen gesucht. F\"ur jeden gefundenen Euler-Weg, werden alle inzidenten Kanten aller Knoten des Weges \"uberpr\"uft. Wird hierbei ein \textit{Graphlet} gefunden, wird der Z\"ahler f\"ur das entsprechende Graphlet um den Wert an der entsprechenden Stelle der unten stehenden Gewichtungsvektoren $w_k$ erh\"oht.


F\"ur \textit{Graphlets} mit $k >= 4$ existieren zus\"atzlich die sogenannten Stern-\textit{Graphlets} ($g_4$ in \ref{fig:4graphlets}, sowie $g_{19}$,$g_{20}$ und $g_{21}$ in \ref{fig:5graphlets}), die keinen solchen Euler-Weg enthalten. Ein Stern-\textit{Graphlet} mit $k$-vielen Knoten hat genau $k$-viele Kanten und keinen Euler-Weg der L\"ange $k-1$.
...
Jede Stelle eines Gewichtungsvektors $w_k$ ist mit einem \textit{Graphlet} assoziiert. Die Nenner der Br\"uche in den Vektoren entsprechen den Anzahlen der Euler-Wege der L\"ange $k-1$ in den entsprechenden \textit{Graphlets}. Wenn alle Euler-Wege eines \textit{Graphlets} besucht wurden und der zugeh\"orige induzierte Teilgraph ihm entspricht wird sein Z\"ahler ganzzahlig und das \textit{Graphlet} gilt als gefunden.
Hierbei bilden  die Stern-\textit{Graphlets}, die keine Euler-Wege der L\"ange $k-1$ enthalten nat\"urlich die Ausnahme.

Somit wird jedes \textit{Graphlet} gez\"ahlt und die entsprechende Anzahl wird in einen Vektor geschrieben. 
Die so erzeugten Vektoren werden im folgenden als \textit{Graphlet}-Vektoren bezeichnet.
Da wir 29 verschiedene \textit{Graphlets} betrachten hat ein \textit{Graphlet}-Vektor 29 Stellen. F\"ur den Vergleich von Proteinstrukturen bedeutet dies, das sich der Berechnugsaufwand auf $O(1)$ reduziert, da anstatt von Graphen oder dreidimensionalen K\"orpern Vektoren im Vektorraum $\mathbb{R}^{29}$ verglichen werden.
...
F\"ur den eingelesenen Graphen berechnet das Programm alle \textit{Graphlets} der Gr\"o"sen 3,4 und 5. Die Implementierung entspricht der Matlab-Implementierung von \textit{N. Shervashidze} \cite{sherv_graphlets}. Zus\"atzlich berechnet es markierte \textit{Graphlets} der Gr\"o"sen 2 und 3, wobei die Markierungen den SSE-Zuordnungen der PTGL entsprechen. Es gibt 35 verschiedene markierte \textit{Graphlets} mit 2 oder 3 Knoten. Unter Ber\"ucksichtigung der Markierungen erh\"oht sich die Anzahl der Stellen der \textit{Graphlet}-Vektoren damit auf 64.
...
\section{\"Ahnlichkeitsma"se}

Durch die Verwendung der \textit{Graphlets} wird der direkte Vergleich von Graphen durch einen Vergleich von \textit{Graphlet}-Vektoren ersetzt. F\"ur den Vergleich dieser Vektoren gibt es prinzipiell zwei verschiedene M\"oglichkeiten. Entweder kann man eine Abstandsmessung durchf\"uhren, oder man z\"ahlt die gemeinsamen Elemente der beiden Vektoren.
...
Zum ersten erm\"oglicht diese Auswahl die beste Vergleichbarkeit der Messungen mit Proteingraphen, Komplexgraphen und Aminos\"auregraphen, da Proteingraphen im Gegensatz zu Komplexgraphen und Amins\"auregraphen nur eine Polypeptidkette darstellen k\"onnen. Durch die Beschr\"ankung auf Proteine mit einer Polypeptidkette wird ein direkter Vergleich durch die Korrelation der paarweisen Distanzen in den unterschiedlichen Formaten m\"oglich.
...
F\"ur diesen Datensatz wurden alle \textit{Graphlet}-Vektoren berechnet. Die paarweisen Distanzen dieser Vektoren zueinander wurden mit der RGF und dem Jaccard-Index berechnet. F\"ur die berechneten Distanzmatrizen wurden die Korrelationen der Distanzen in den verschiedenen Formaten berechnet und sie wurden mit den von PDBeFold berechneten Distanzen verglichen. 
...
\section{Bewertungsfunktionen in PLCC}
